	...
	public GPolygon(GPoint[] points) {
		...
		synchronized (lock) {
			for (int i = 0; i < points.length; i++) {
				vertexList.add(new GPoint(points[i].getX(), points[i].getY()));
			}
		}
		markAsComplete();
		rotation = 0;
 ...
 */
	public void addEdge(double dx, double dy) {
		...
		synchronized (lock) {
			cx += dx;
			cy += dy;
			vertexList.add(new GPoint(cx, cy));
		}
	}
 ...
 */
	public final void addPolarEdge(double r, double theta) {
		...
		synchronized (lock) {
			cx += r * GMath.cosDegrees(theta);
			cy -= r * GMath.sinDegrees(theta);
			vertexList.add(new GPoint(cx, cy));
		}
	}
 ...
 */
	public GPoint getCurrentPoint() {
		return (vertexList.isEmpty()) ? null : new GPoint(cx, cy);
	}
	...
	public GRectangle getBounds() {
		...
		synchronized (lock) {
			...
			for (int i = 0; i < vertexList.size(); i++ ) {
				GPoint vertex = (GPoint) vertexList.get(i);
 ...
 */
	public boolean contains(double x, double y) {
		return getPolygon().contains(GMath.round(x), GMath.round(y));
	}
...
/**
 * Returns an AWT <code>Polygon</code> whose point are as close as possible
...
/* Private method: copyVertexList() */
/**
 * Makes a copy of the vertex list to ensure that clones are independent.
 * @noshow
 */
	private void copyVertexList() {
		ArrayList newList = new ArrayList();
		for (int i = 0; i < vertexList.size(); i++) {
			newList.add(vertexList.get(i));
		}
		vertexList = newList;
	}
