/*
 * @(#)GraphicsProgram.java   1.0 06/08/25
 */
 ...
 */
	protected GraphicsProgram() {
		listener = new GProgramListener(this);
		gc = createGCanvas();
		gc.addMouseListener(listener);
		if (listener.needsMouseMotionListeners()) {
 ...
 * you are defining your own <code>GraphicsProgram</code> class, you need to
 ...
 *
 * <code>&nbsp;&nbsp;for (Iterator i = iterator(direction); i.hasNext(); )</code>
 *
 * <p>Applets that want to run in browsers, however, should avoid using
 * this method, because <code>Iterator</code> is not supported on 1.1 browsers.
 * For maximum portability, you should rely instead on the
 ...
 * @param Sets the background color of the underlying <code>GCanvas</code>
 * @noshow
 */
...
/**
 * Ensures that the menu bar appears when the program starts.
 */
...
/* Method: resetClickListener() */
/**
 * Resets the click flag at the beginning of the <code>waitForClick</code> cycle.
 */
	public void resetClickListener() {
		clickFlag = false;
	}

/* Method: clickOccurred() */
/**
 * Tests to see whether a click has occurred.
 */
