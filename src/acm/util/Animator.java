...
/* Unadvertised method: registerSpeedBar(speedBar) */
/**
 * Registers the specified speed bar as the delay controller for the animator.
 * The methods is required for the compatability code, since JSlider and JScrollBar
 * will be different types.
 *
 * @usage registerSpeedBar(speedBar);
 * @param speedBar The speed bar for this animator
 * @noshow
 */
	public void registerSpeedBar(Object speedBar) {
		SpeedBarListener.register(this, speedBar);
	}
 ...
 */
	private void initAnimator() {
		...
		if (applet != null) JTFTools.registerApplet(applet, this);
	}
 ...
 */
	public static void register(Animator animator, Object speedBar) {
		...
		boolean useFakeListener = false;
		Method addListener = lookForListener(speedBarClass, "addAdjustmentListener", "java.awt.event.AdjustmentListener");
		if (addListener == null) {
			addListener = lookForListener(speedBarClass, "addChangeListener", "javax.swing.event.ChangeListener");
		}
		if (addListener == null) {
			addListener = lookForListener(speedBarClass, "addChangeListener", "ChangeListener");
			useFakeListener = true;
		}
		try {
			...
			if (useFakeListener) {
				Class fakeChangeListener = Class.forName("FakeChangeListener");
				Class[] types = { Class.forName("java.awt.event.AdjustmentListener") };
				Constructor newFakeChangeListener = fakeChangeListener.getConstructor(types);
				args[0] = newFakeChangeListener.newInstance(args);
			}
...
/* Private static method: lookForListener(speedBarClass, methodName, parameterClassName) */
/**
 * Looks for a method in the class with the appropriate name and parameter class.
 */
	private static Method lookForListener(Class speedBarClass, String methodName, String parameterClassName) {
		try {
			Class[] types = { Class.forName(parameterClassName) };
			return speedBarClass.getMethod(methodName, types);
		} catch (Exception ex) {
			return null;
		}
	}
