 ...
 */
	public IOConsole() {
		...
		add(consoleModel.getConsolePane(), BorderLayout.CENTER);
...
/* Package class: ConsoleModel */

/**
 * This class implements the console model using Swing.
 */
class ConsoleModel implements KeyListener, FocusListener {
 ...
 */
	public void print(PrintJob pj) {
		...
		for (int page = 1; page <= nPages; page++) {
			if (page > 1) g = pj.getGraphics();
			int bottom = getLineY(text, 1 + page * linesPerPage);
			g.translate(0, -top);
 ...
 */
	public void focusGained(FocusEvent e) {
		hasFocus = true;
		ProgramMenuBar mbar = findMenuBar();
		if (mbar != null) {
			mbar.setConsole(console);
			updateMenuBar(mbar);
		}
	}
...
/* Private method: updateMenuBar(mbar) */
/**
 * Updates the program menu bar to enable the appropriate menu items.
 */
...
/* Private method: countLines(text) */
/**
 * Counts the number of lines in <code>text</code>.
 */
	private int countLines(String text) {
		int count = 0;
		for (int pos = -1; (pos = text.indexOf(lineSeparator, pos + 1)) != -1; count++) {
			/* Empty */
		}
		if (!text.endsWith(lineSeparator)) count++;
		return count;
	}

/* Private method: getLineY(text, lineNumber) */
/**
 * Returns the y coordinate of the top of the specified line (numbering from 1).
 */
	private int getLineY(String text, int lineNumber) {
		...
		try {
			for (int i = 1; i < lineNumber; i++) {
				int eol = text.indexOf(lineSeparator, pos);
				if (eol == -1) {
					Rectangle bounds = textPane.modelToView(pos);
					return bounds.y + bounds.height;
				}
				pos = eol + lineSeparator.length();
			}
			return textPane.modelToView(pos).y;
		} catch (Exception ex) {
			throw new ErrorException("Internal error: " + ex);
		}
	}

/* Private method: findMenuBar() */
/**
 * Finds the menu bar associated with this console.  This search is complicated,
 * since it is possible for a console to be embedded in a <code>JFrame</code> without an
 * intervening program.
 */
...
/* Constructor: AWTConsoleModel() */
/**
 * Creates a new <code>AWTConsoleModel</code> object.
 */
